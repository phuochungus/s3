...
//     const enckey = key.split('/').map(x => encodeURIComponent(x)).join('/');
//     const rc = `https://s3.amazonaws.com/${bucket}/${enckey}`;
//     return escape ? htmlEscape(rc) : rc;
// }
...
function UploadController($scope, SharedService) {
    ...
    $scope.uploadFiles = (Bucket, prefix) => {
        ...
        $scope.upload.files.forEach(async (file, ii) => {
            DEBUG.log('File:', file);
            DEBUG.log('Index:', ii);
            ...
            const s3 = new AWS.S3(AWS.config);
            DEBUG.log('AWS.config: ==========', AWS.config)
...
$(document).ready(() => {
    ...
    moment().format();
    angular.element(document).injector().get('SharedService').changeSettings(settings);
});
